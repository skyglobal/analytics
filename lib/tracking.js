...
skytoolkit['trackingSetup'] = function(omniture){
...
//    not using jQuery .parents([data-tarcking-whatever]) as is slow in ie and ff
    function checkParentForAttribute(el, attr){
        ...
        checkParentForAttribute(el.parentNode, attr);
    }
...
function init(){
    if (skytoolkit['omniture']){
        window.skytoolkit['tracking'] = skytoolkit['trackingSetup'](skytoolkit['omniture']);
    } else {
        setTimeout(init,125);
    }
}

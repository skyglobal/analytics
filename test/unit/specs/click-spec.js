function clickSpec(lc) {
    ...
    describe(describeSpec, function () {
        ...
        it('with data-tracking-label first', function () {
            expect(lc.getText(createFixtureElement('div', dataFixture.dataTrackingLabel.div))).to.equal('this is my div-tracking-label label');
            expect(lc.getText((createFixtureElement('input', dataFixture.dataTrackingLabel.input)))).to.equal('this is my input-tracking-label label');
        });

        it('data-tracking-value next', function () {
            expect(lc.getText(createFixtureElement('div', dataFixture.dataTrackingValue.div))).to.equal('this is my div-tracking-label value');
            expect(lc.getText(createFixtureElement('input', dataFixture.dataTrackingValue.input))).to.equal('this is my input-tracking-label value');
        });

        it('then alt attribute', function () {
            expect(lc.getText(createFixtureElement('div', dataFixture.alt.div))).to.equal('this is my div-alt alt');
            expect(lc.getText(createFixtureElement('input', dataFixture.alt.input))).to.equal('this is my input-alt alt');
        });

        it('then the current input value before the original value attribute', function () {
            expect(lc.getText(createFixtureElement('div', dataFixture.value.div))).to.equal('this is my div-value name');//this is ok as div's shouldn't have value attribute
            var $el = createFixtureElement('input', dataFixture.value.input);
            expect(lc.getText($el)).to.equal('this is my input-value value');
            $el.val('pete');
            expect(lc.getText($el)).to.equal('pete');
            $el.val('');
            expect(lc.getText($el)).to.equal('this is my input-value name');
        });

        it('and for backwards compatibility, the name attribute', function() {
            expect(lc.getText((createFixtureElement('div', dataFixture.name.div)))).to.equal('this is my div-name name');
            expect(lc.getText((createFixtureElement('input', dataFixture.name.input)))).to.equal('this is my input-name name');
        });

        it('and finally the text within the tag', function() {
            expect(lc.getText((createFixtureElement('div',dataFixture.text.attr).text(dataFixture.text.text)))).to.equal('this is my div-text text');
...
if (window.define) {
    define('specs/click-spec', ['core/track-click'], function (lc) {
        return clickSpec(lc);
    });
}
